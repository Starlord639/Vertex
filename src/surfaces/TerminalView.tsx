...
export function TerminalView({ id, cwd, active, onMetaChange }: Props) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      observer.disconnect();
      outer.classList.remove("vertex-terminal--alt-screen");
  ...
  return (
    <div
      ref={outerRef}
      className="vertex-terminal flex h-full w-full min-h-0 min-w-0 flex-col"
      onMouseDown={() => termRef.current?.focus()}
    >
      <div
        ref={hostRef}
        className="vertex-terminal-host min-h-0 min-w-0 flex-1 overflow-hidden"
      />
    </div>
  );
}
