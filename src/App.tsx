...
function confirmDiscardUnsaved(message: string): Promise<boolean> {
  return ask(message, { title: "Vertex", kind: "warning" });
}
...
export default function App({
  ...
  const onSubmit = useCallback(
    (
      ...
      const rawCommand =
        !chatOnly && isNativeCommandPrompt(submittedText, current.harness);
      ...
      void (async () => {
        ...
        try {
          ...
          const prompt = chatOnly
            ? [
                "[Vertex Chat mode: This is a conversation-only turn. Do not create, edit, rename, move, or delete files, and do not run commands that can change the workspace. You may read existing context when needed, but respond conversationally only.]",
                preparedPrompt,
              ].join("\n\n")
            : preparedPrompt;
          const turnPrompt =
          ...
          await sendHarnessTurn({
            ...
            model: current.model,
            modelSettings: current.modelSettings,
            runtimeMode: chatOnly ? "supervised" : current.runtimeMode,
            intent,
            text: inboxAskPrompt(
              rawCommand ? undefined : current.inboxAsk,
              wrap && !rawCommand
            ...
            onEvent: (event) => {
              if (turnGen.current.get(sessionId) !== gen) return;
              if (chatOnly && event.type === "approval.requested") {
                respondHarnessApproval(
                  current.harness,
                  sessionId,
                  event.requestId,
  ...
  useEffect(() => {
    const onEscape = (event: KeyboardEvent) => {
      ...
      const inTerminal = Boolean(target?.closest(".vertex-terminal"));
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (cmd) {
        ...
        if (listNavigation) {
          const blockedTarget = Boolean(
            target?.closest(
              'input, textarea, select, [contenteditable="true"], .cm-editor, .vertex-terminal, [role="dialog"], [data-model-picker], [data-file-picker], [data-branch-picker], [data-skill-picker], [data-mention-picker], [data-app-search]',
            ),
          );
        ...
        if (
          target?.closest(".vertex-terminal") &&
          e.ctrlKey &&
          !e.metaKey &&
          (cmd === "back" ||
            cmd === "forward" ||
      ...
      if (mod && !e.altKey && !e.shiftKey && e.key.toLowerCase() === "k") {
        const target = e.target instanceof Element ? e.target : null;
        if (target?.closest(".vertex-terminal") && e.ctrlKey && !e.metaKey) {
          return;
        }
        e.preventDefault();
        e.stopPropagation();
