...
function session(id: string, cwd = "/projects/vertex"): Session {
...
describe("removeSessionFromWorkspace", () => {
  ...
  it("does not leave the project when closing its final tab in project scope", () => {
    const result = remove({
      tabs: [tab("ruler", "r1"), tab("vertex", "s1")],
    ...
    expect(result.tabs.map((entry) => entry.id)).toEqual(["ruler", "vertex"]);
    expect(result.activeTabId).toBe("vertex");
    expect(result.tabs[1]?.focusedId).toBe("replacement");
  });
  it("removes only the archived session's plans in a shared workspace", () => {
    const own = newPlanTab("s1", "p1", "Own plan", "/projects/vertex");
    const other = newPlanTab("s2", "p2", "Other plan", "/projects/vertex");
    const readme = newFileTab(
      "/projects/vertex/README.md",
      "/projects/vertex",
    );
  ...
  it("removes plan panes even after their conversation moved to another tab", () => {
    const plan = newPlanTab("s1", "p1", "Plan", "/projects/vertex");
