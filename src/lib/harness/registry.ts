...
export type HarnessAdapter = {
  ...
  /** Trigger provider-owned compaction outside Vertex's normal user-turn path. */
  ...
  /** Seed resume state from a restored Vertex session. */
    ...
    [...adapters.values()]
      ...
      .map(async (adapter) => {
        ...
        await adapter.refreshCatalog().catch((error: unknown) => {
          console.debug(`[vertex] ${adapter.id} catalog`, error);
        });
      }),
  );
}
