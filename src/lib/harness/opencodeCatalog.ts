...
export function refreshOpenCodeCatalog(): Promise<void> {
  ...
  inflight = discoverOpenCodeModels()
    ...
    .catch((error: unknown) => {
      console.debug("[vertex] opencode catalog", error);
    })
    .finally(() => {
      inflight = null;
    });
...
async function discoverOpenCodeModels(): Promise<AgentModel[]> {
  ...
  if (!version) {
    throw new Error(
      `Unable to determine OpenCode version. Vertex requires v${MINIMUM_OPENCODE_VERSION} or newer.`,
    );
  }
  ...
  try {
    ...
    console.debug("[vertex] opencode agents", error);
  }
