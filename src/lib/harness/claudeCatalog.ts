...
const PROBE_ID = "vertex-claude-probe";
const LIST_MODELS_REQUEST_ID = "vertex_list_models";
const INIT_REQUEST_ID = "vertex_init";
...
export function refreshClaudeCatalog(): Promise<void> {
  ...
  inflight = discoverClaudeModels()
    ...
    .catch((error: unknown) => {
      console.debug("[vertex] claude catalog", error);
    })
    .finally(() => {
      inflight = null;
    });
...
async function discoverClaudeModels(): Promise<AgentModel[]> {
  const listed = await discoverViaListModels().catch((error: unknown) => {
    console.debug("[vertex] claude list_models catalog failed", error);
    return [];
  });
