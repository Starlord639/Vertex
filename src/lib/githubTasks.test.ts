  ...
  it("keeps one card per GitHub issue across local checkouts", () => {
    const rows = [
      item({
        ...
        repo: "hardbeat920/vertex",
      }),
      item({
        number: 10,
        updatedAt: "2026-08-27T10:00:00Z",
        projectPath: "/tmp/vertex",
        repo: "HardBeat920/vertex",
      }),
    ];
    const deduped = dedupeInboxItems(rows, ["/tmp/vertex", "/tmp/agent-terminal"]);
    expect(deduped).toHaveLength(1);
    expect(deduped[0]?.projectPath).toBe("/tmp/vertex");
    expect(inboxItemKey(deduped[0]!)).toBe(
      "github:hardbeat920/vertex:issue:10",
    );
  });
});
...
describe("groupProjectsByRepo", () => {
  it("fetches each GitHub remote once", () => {
    expect(
      groupProjectsByRepo([
        { path: "/tmp/vertex", repo: "hardbeat920/vertex" },
        { path: "/tmp/agent-terminal", repo: "HardBeat920/vertex" },
        { path: "/tmp/docs", repo: "acme/docs" },
      ]).map((project) => project.path),
    ).toEqual(["/tmp/vertex", "/tmp/docs"]);
  });
});
