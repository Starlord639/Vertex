...
describe("project appearance keys", () => {
  beforeEach(() => {
    mockLocalStorage({ "vertex:tab-group:key-version": "2" });
  });
...
describe("migrateProjectAppearanceKeys", () => {
  it("moves folder-name entries onto every project that carries the name", () => {
    const store = mockLocalStorage({
      "vertex.recentProjects": JSON.stringify([
        { path: FINANCE, openedAt: 2 },
        { path: CORTEX, openedAt: 1 },
      ]),
      "vertex:tab-group:labels": JSON.stringify({ agentbase: "Agentbase" }),
      "vertex:tab-group:colors": JSON.stringify({ agentbase: "4" }),
    });
    ...
    expect(store.get("vertex:tab-group:key-version")).toBe("2");
  ...
  it("leaves entries for projects it no longer knows about", () => {
    const store = mockLocalStorage({
      "vertex:tab-group:labels": JSON.stringify({ gone: "Gone" }),
    });
    expect(loadTabGroupLabels().gone).toBe("Gone");
    // Unfinished: a later launch must still get the chance to claim it.
    expect(store.get("vertex:tab-group:key-version")).toBeUndefined();
  });
  ...
  it("claims an entry once its project is remembered again", () => {
    ...
    mockLocalStorage({
      "vertex:tab-group:labels": JSON.stringify({ agentbase: "Finance" }),
    });
    expect(loadTabGroupLabels()[projectKey(FINANCE)]).toBeUndefined();

    // Next launch, with the project reopened.
    mockLocalStorage({
      "vertex.recentProjects": JSON.stringify([{ path: FINANCE, openedAt: 1 }]),
      "vertex:tab-group:labels": JSON.stringify({ agentbase: "Finance" }),
    });
