...
pub(crate) fn ensure_dev_bundle() {
    if let Err(err) = relaunch_from_dev_bundle() {
        eprintln!("vertex: macos dev bundle: {err}");
    }
}
...
fn relaunch_from_dev_bundle() -> Result<(), String> {
    ...
    let app = exe.parent().ok_or("missing exe parent")?.join("Vertex.app");
    let macos_dir = app.join("Contents/MacOS");
    std::fs::create_dir_all(&macos_dir).map_err(|e| e.to_string())?;
    write_dev_bundle_icons(&app)?;

    let bundled = macos_dir.join("vertex");
    ...
    // The linker's ad-hoc signature carries a `vertex-<hash>` identifier.
    ...
    let signed = Command::new("/usr/bin/codesign")
        ...
        .unwrap_or(false);
    if !signed {
        eprintln!("vertex: macos dev bundle: codesign failed; notifications stay off");
    }
...
#[cfg(debug_assertions)]
const DEV_BUNDLE_ID: &str = "com.vertex.desktop";
#[cfg(debug_assertions)]
...
<dict>
	...
	<string>com.vertex.desktop</string>
