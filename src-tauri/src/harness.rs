...
const HARNESS_PARENT_ENV: &str = "VERTEX_HARNESS_PARENT";
...
/// Kill harness trees left behind by a previous Vertex that exited
...
mod windows_launcher_tests {
    ...
    fn npm_shell_shim_does_not_hide_windows_launcher() {
        let dir = std::env::temp_dir().join(format!("vertex-launcher-{}", std::process::id()));
...
mod tests {
    ...
    fn which_in_path_takes_the_first_executable_hit() {
        ...
        let dir = std::env::temp_dir().join(format!("vertex-which-{}", std::process::id()));
    ...
    fn resolve_gui_binary_finds_a_binary_on_the_gui_path() {
        ...
        let dir = std::env::temp_dir().join(format!("vertex-gui-bin-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn cursor_agent_accepts_symlink_named_agent() {
        let dir = std::env::temp_dir().join(format!("vertex-agent-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn pi_accepts_coding_agent_and_rejects_other_pi() {
        let dir = std::env::temp_dir().join(format!("vertex-pi-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn omp_accepts_rpc_capable_binary_and_rejects_other_names() {
        let dir = std::env::temp_dir().join(format!("vertex-omp-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn fx_accepts_vercel_agent_and_rejects_json_viewer() {
        let dir = std::env::temp_dir().join(format!("vertex-fx-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn fx_marker_is_found_past_the_first_chunk() {
        let dir = std::env::temp_dir().join(format!("vertex-fx-deep-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn grok_accepts_official_install_path_and_markers() {
        let dir = std::env::temp_dir().join(format!("vertex-grok-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
...
mod reap_logic_tests {
    ...
    fn parse_ps_row_reads_harness_parent_from_env_tail() {
        let parsed = parse_ps_row(
            " 27129 21504 /Users/n/cursor-agent acp PATH=/usr/bin VERTEX_HARNESS_PARENT=21504 HOME=/tmp",
        )
        .unwrap();
    ...
    fn parse_ps_pid_command_does_not_treat_the_binary_as_ppid() {
        let (pid, command) = parse_ps_pid_command(
            " 27129 /Users/n/cursor-agent acp PATH=/usr/bin VERTEX_HARNESS_PARENT=21504 HOME=/tmp",
        )
        .unwrap();
        assert_eq!(pid, 27129);
