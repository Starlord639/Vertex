...
fn with_temp_markdown(
    ...
    let stamp = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let path = std::env::temp_dir().join(format!("vertex-comment-{stamp}.md"));
...
fn git_pr_create_for(root: &Path, input: &GitPrCreateInput) -> Result<String, String> {
    ...
    let stamp = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let body_path = std::env::temp_dir().join(format!("vertex-pr-{stamp}.md"));
...
fn write_attachment_sync(name: &str, data: &str) -> Result<String, String> {
    ...
    let dir = std::env::temp_dir().join("vertex-attachments");
    std::fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
    let stamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
...
mod tests {
    ...
    fn editor_text_files_round_trip_without_leaving_a_temporary_file() {
        let stamp = SystemTime::now()
            ...
            .as_nanos();
        let dir =
            std::env::temp_dir().join(format!("vertex-editor-{}-{stamp}", std::process::id()));
    ...
    fn init_git(dir: &Path, branch: &str, origin: Option<&str>) -> bool {
        ...
        git(dir, &["config", "user.name", "Vertex"])
            && git(dir, &["config", "user.email", "vertex@test"])
    ...
    fn git(dir: &Path, args: &[&str]) -> bool {
        Command::new("git")
            .args([
                ...
                "user.name=Vertex",
                "-c",
                "user.email=vertex@test",
                "-c",
                "commit.gpgsign=false",
            ])
            .args(args)
            .current_dir(dir)
            .env("GIT_AUTHOR_NAME", "Vertex")
            .env("GIT_AUTHOR_EMAIL", "vertex@test")
            .env("GIT_COMMITTER_NAME", "Vertex")
            .env("GIT_COMMITTER_EMAIL", "vertex@test")
            .status()
    ...
    fn git_sync_pulls_then_pushes() {
        ...
        if !git(&a.0, &["remote", "add", "origin", &origin_url])
            ...
            || Command::new("git")
                ...
                .map(|status| !status.success())
                .unwrap_or(true)
            || !git(&b.0, &["config", "user.name", "Vertex"])
            || !git(&b.0, &["config", "user.email", "vertex@test"])
    ...
    fn pr_head_filter_qualifies_branch_with_repo_owner() {
        assert_eq!(
            github_pr_head_filter("hardbeat920/vertex", "main").as_deref(),
    ...
    fn split_github_repo_reads_owner_and_name() {
        assert_eq!(
            split_github_repo(" hardbeat920/vertex ").unwrap(),
            ("hardbeat920".into(), "vertex".into())
        );
        assert!(split_github_repo("vertex").is_err());
