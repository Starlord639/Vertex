...
/// Restrict DLL lookup before the first PTY is opened. Vertex itself stays
...
mod tests {
    ...
    fn abrupt_exit_kills_children() {
        const MARKER: &str = "VERTEX_JOB_TEST_CHILD";
        const PID_FILE: &str = "VERTEX_JOB_TEST_DESCENDANT_FILE";
        ...
        if std::env::var_os(MARKER).is_some() {
            ...
            let pid_file = std::env::temp_dir()
                .join(format!("vertex-pty-descendant-{}.pid", std::process::id()));
